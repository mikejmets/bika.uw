# This file is part of Bika UW LIMS
#
...
class SimpleTestLayer(PloneSandboxLayer):
    ...
    def setUpPloneSite(self, portal):
        ...
        applyProfile(portal, 'bika.uw:default')
...
BIKA_UW_SIMPLE_FIXTURE = SimpleTestLayer()
BIKA_UW_SIMPLE_FIXTURE['getBrowser'] = getBrowser
BIKA_UW_SIMPLE_TESTING = FunctionalTesting(
    bases=(BIKA_UW_SIMPLE_FIXTURE,),
    name="SimpleTestingLayer:Functional"
)


class BikaUWTestLayer(SimpleTestLayer):
    def setUpZope(self, app, configurationContext):
        super(BikaUWTestLayer, self).setUpZope(app, configurationContext)
...
class Keywords(object):
    ...
    def resource_filename(self):
        res = pkg_resources.resource_filename("bika.uw", "tests")
        return res
...
class RemoteKeywords(Keywords, RemoteLibrary):
    ...
    def create_object(self, path, portal_type, id, **kwargs):
        portal = api.portal.get()
        container = portal.unrestrictedTraverse(path.strip('/').split('/'))
        # create object
...
BIKA_UW_FUNCTIONAL_FIXTURE = BikaUWTestLayer()
BIKA_UW_FUNCTIONAL_FIXTURE['getBrowser'] = getBrowser
BIKA_UW_FUNCTIONAL_TESTING = FunctionalTesting(
    bases=(BIKA_UW_FUNCTIONAL_FIXTURE,),
    name="BikaUWTestingLayer:Functional"
)
...
BIKA_UW_ROBOT_TESTING = FunctionalTesting(
    bases=(BIKA_UW_FUNCTIONAL_FIXTURE,
           REMOTE_FIXTURE,
           z2.ZSERVER_FIXTURE),
    name="BikaUWTestingLayer:Robot"
)
